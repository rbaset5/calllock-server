...
import { sendJobToDashboard, isDashboardEnabled, sendCallToDashboard, sendEmergencyAlertToDashboard, transformToDashboardPayload, sendPayloadToDashboard } from "./services/dashboard.js";
import {
...
function extractStateFromPostCallData(callData: RetellPostCallData): ConversationState {
  ...
  return {
    ...
    // Diagnostic context
    problemOnset: custom?.problem_onset,
    customerAttemptedFixes: custom?.customer_attempted_fixes,
    // Triage fields
    callerType: custom?.caller_type as CallerType | undefined,
    primaryIntent: custom?.primary_intent as PrimaryIntent | undefined,
    isCallbackComplaint: custom?.is_callback_complaint,
...
app.post("/webhook/retell/call-ended", async (req: Request, res: Response) => {
  try {
    ...
    // We still sync to dashboard (for false-positive review) but mark as archived
    if (
      conversationState.callerType === "vendor" ||
      conversationState.callerType === "recruiting" ||
      conversationState.primaryIntent === "solicitation"
    ) {
      logger.info({ callId, callerType: conversationState.callerType, primaryIntent: conversationState.primaryIntent }, "Routing to spam folder");

      // Transform and add archived flags
      const archivedPayload = transformToDashboardPayload(conversationState, payload.call);
      archivedPayload.status_color = "gray";
      archivedPayload.is_archived = true;

      // Send archived payload to dashboard
      const archiveResult = await sendPayloadToDashboard(archivedPayload);

      // Also sync call record (non-blocking)
      ...
      return res.json({ success: true, action: "archived_spam" });
    }

    // Normal flow - Send to dashboard (job/lead)
...
app.post("/webhook/retell/book_appointment", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result.success && result.appointmentId) {
      ...
      // Auto-set bookingStatus on successful booking (Phase 2 Triage)
...
app.post("/webhook/retell/end_call", async (req: Request, res: Response) => {
  try {
    ...
    // V3 - Validate triage args with Zod (graceful fallback to defaults)
    const validated = EndCallArgsSchema.safeParse(args);
    if (!validated.success) {
      logger.warn(
        { callId: call.call_id, errors: validated.error.flatten() },
        "Invalid end_call args - using defaults for invalid fields"
      );
    }
    // Use validated data or parse with defaults for invalid fields
    const safeArgs = validated.success
      ? validated.data
      : EndCallArgsSchema.parse({ reason: (args as Record<string, unknown>).reason || "unknown" });

    const reason = safeArgs.reason;
    ...
    if (safeArgs.customer_name) state.customerName = safeArgs.customer_name;
    if (safeArgs.customer_phone) state.customerPhone = safeArgs.customer_phone;
    if (safeArgs.customer_address) state.serviceAddress = safeArgs.customer_address;
    if (safeArgs.problem_description) state.problemDescription = safeArgs.problem_description;
    if (safeArgs.urgency) state.urgency = safeArgs.urgency as UrgencyLevel;

    // Capture diagnostic context fields
    if (safeArgs.problem_duration) state.problemDuration = safeArgs.problem_duration;
    if (safeArgs.problem_onset) state.problemOnset = safeArgs.problem_onset;
    if (safeArgs.problem_pattern) state.problemPattern = safeArgs.problem_pattern;
    if (safeArgs.customer_attempted_fixes) state.customerAttemptedFixes = safeArgs.customer_attempted_fixes;

    // Capture equipment details
    if (safeArgs.equipment_type) state.equipmentType = safeArgs.equipment_type;
    if (safeArgs.equipment_brand) state.equipmentBrand = safeArgs.equipment_brand;
    if (safeArgs.equipment_location) state.equipmentLocation = safeArgs.equipment_location;
    if (safeArgs.equipment_age) state.equipmentAge = safeArgs.equipment_age;

    // V3 TRIAGE - Capture AI Classifications (validated by Zod)
    // Defaults: caller_type="unknown", is_callback_complaint=false
    state.callerType = safeArgs.caller_type;
    if (safeArgs.primary_intent) state.primaryIntent = safeArgs.primary_intent;
    state.isCallbackComplaint = safeArgs.is_callback_complaint;

    // BOOKING STATUS - Trust System State over LLM claims
    // The LLM might hallucinate "confirmed" even if the book_appointment tool failed
    if (safeArgs.booking_status) state.bookingStatus = safeArgs.booking_status;

    // OVERRIDE: If book_appointment actually succeeded, we have an appointmentId
    if (state.appointmentId) {
      state.bookingStatus = "confirmed";
    }

    // SAFETY: If AI claims confirmed but no appointmentId exists, downgrade
    if (state.bookingStatus === "confirmed" && !state.appointmentId) {
      logger.warn(
        { callId: state.callId },
        "AI claimed booking confirmed but no appointmentId found. Downgrading to attempted_failed."
      );
      state.bookingStatus = "attempted_failed";
    }
