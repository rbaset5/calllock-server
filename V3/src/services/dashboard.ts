...
import { ConversationState, UrgencyTier, EndCallReason, RetellPostCallData, RevenueTier, CallerType, PrimaryIntent, BookingStatus } from "../types/retell.js";
...
/**
 * Calculate status color for visual priority strips
 * | Color  | Meaning                 | Condition                                     |
 * |--------|-------------------------|-----------------------------------------------|
 * | red    | Emergency/Urgent        | urgency=emergency OR isCallbackComplaint=true |
 * | green  | Commercial (High Value) | callerType=commercial                         |
 * | gray   | Low Priority/Spam       | Info-only, out of area, archived              |
 * | blue   | General Lead            | Standard new leads (default)                  |
 */
function calculateStatusColor(state: ConversationState): DashboardJobPayload["status_color"] {
  // Emergency or safety = red
  if (state.urgencyTier === "LifeSafety" || state.isSafetyEmergency) return "red";
  // Callback complaints = red (upset customer needs urgent attention)
  if (state.isCallbackComplaint) return "red";
  // Urgent escalation = red
  if (state.isUrgentEscalation) return "red";
  // Commercial callers = green ($$ opportunity)
  if (state.callerType === "commercial") return "green";
  // Out of service area = gray
  if (state.endCallReason === "out_of_area") return "gray";
  // Default = blue
  return "blue";
}
...
export function transformToDashboardPayload(
  ...
  return {
    ...
    // Triage Engine fields
    caller_type: state.callerType,
...
/**
 * Send a pre-built payload to dashboard (for Traffic Controller use)
 * Unlike sendJobToDashboard, this takes a ready-made payload with archived flags
 */
export async function sendPayloadToDashboard(
  payload: DashboardJobPayload
): Promise<{ success: boolean; jobId?: string; error?: string }> {
  if (!isDashboardConfigured) {
    log.warn({ callId: payload.call_id }, "Dashboard not configured - skipping sync");
    return { success: false, error: "Dashboard not configured" };
  }
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      log.error({ status: response.status, error: errorText }, "Dashboard payload webhook failed");
      return { success: false, error: `HTTP ${response.status}: ${errorText}` };
    }

    const result = await response.json() as { success: boolean; job_id?: string };
    log.info({ jobId: result.job_id, isArchived: payload.is_archived }, "Payload sent to dashboard");
    return { success: true, jobId: result.job_id };
  } catch (error) {
    if (error instanceof FetchError) {
      log.error({ error: error.message }, "Dashboard payload webhook failed after retries");
      return { success: false, error: error.message };
    }
    log.error({ error }, "Dashboard payload webhook error");
    return { success: false, error: String(error) };
  }
}
